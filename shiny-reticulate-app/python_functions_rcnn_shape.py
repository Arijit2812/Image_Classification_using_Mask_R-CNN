...
# create config
cfg_sh = PredictionConfig()
# define the model
model_sh = MaskRCNN(mode='inference', model_dir='C:/Users/BhattacharjeeA/shiny-reticulate-app', config=cfg_sh)

# load model weights
model_sh.load_weights('C:/Users/BhattacharjeeA/shiny-reticulate-app/mask_rcnn_hoof_shape_cfg_coco_0023.h5', by_name=True)

#Function to test image
def test_shape_image(image_path=None):
    hoof_img = skimage.io.imread(image_path)
    detected = model_sh.detect([hoof_img])
    results = detected[0]
